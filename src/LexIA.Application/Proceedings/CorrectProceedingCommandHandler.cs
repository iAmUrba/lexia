...
public sealed class CorrectProceedingCommandHandler : ICommandHandler<CorrectProceedingCommand>
{
    private readonly IEventStore _eventStore;
    private readonly IProceedingProjectionDispatcher _projectionDispatcher;

    public CorrectProceedingCommandHandler(IEventStore eventStore, IProceedingProjectionDispatcher projectionDispatcher)
    {
        _eventStore = eventStore;
        _projectionDispatcher = projectionDispatcher;
    }

    public async Task HandleAsync(CorrectProceedingCommand command, CancellationToken cancellationToken = default)
    {
        ...
        {
            if (uncommitted.Event is ProceedingCorrected e)
            {
                await _projectionDispatcher.DispatchCorrectedAsync(e, cancellationToken);
            }
        }
    }
}
