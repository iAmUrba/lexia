...
// Mock the DB for this test to bypass native binding issues in this environment
jest.mock('./db/index.js', () => ({
  db: {
    select: () => ({
      from: () => ({
        where: () => ({
          get: () => null // Simula que no está en la base de datos local para forzar a Graph API
        })
      })
    }),
    insert: () => ({
...
async function runSprint2Test() {
  ...
  // 1. Simular Documento desde testdata
  ...
  // Hacemos override del `db` importado dentro de resolver para este script de prueba
  (resolver as any).db = require('./db/index.js').db;
...
// Ignore TS errors for jest in this standalone script
runSprint2Test();
